...
uniform float vertexSnapStrength;
...
void main()
{
    ...
    vec2 snappedScreenPosition = floor(screenPosition + 0.5);
    vec2 snappedNdc = (snappedScreenPosition/virtualResolution)*2.0 - 1.0;

    ndcPosition.xy = mix(ndcPosition.xy, snappedNdc, clamp(vertexSnapStrength, 0.0, 1.0));
