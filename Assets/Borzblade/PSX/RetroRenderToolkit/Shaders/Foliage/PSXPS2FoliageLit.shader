Shader "Borzblade/Retro Render Toolkit/PSX PS2 Foliage Lit"
{
    Properties
    {
        ...
        [Toggle(_RETRO_FOLIAGE_WIND)] _WindEnabled("Wind Enabled", Float) = 1
        _WindStrength("Wind Strength", Range(0, 1)) = 0.18
        _WindDistance("Jiggle Amplitude", Range(0, 2)) = 0.06
        _WindSpeed("Wind Speed", Range(0, 10)) = 1.4
        _WindScale("Wind Scale", Range(0.05, 8)) = 1.2
        _WindDirection("Wind Direction", Vector) = (1, 0.25, 0, 0)
        _WindVertexColorMask("Vertex Color Alpha Wind Mask", Range(0, 1)) = 0
        _WindUVHeightMask("UV Height Wind Mask", Range(0, 1)) = 1

        [Toggle(_RETRO_FOLIAGE_GRADIENT)] _FoliageGradientEnabled("Top/Bottom Gradient", Float) = 1
        _FoliageTopColor("Top Color", Color) = (0.72, 0.88, 0.46, 1)
        _FoliageBottomColor("Bottom Color", Color) = (0.20, 0.32, 0.14, 1)
        [Toggle(_RETRO_FOLIAGE_VARIATION)] _FoliageVariationEnabled("Color Variation", Float) = 1
        _FoliageVariationStrength("Variation Strength", Range(0, 1)) = 0.18
        _HueVariation("Hue Variation", Range(0, 1)) = 0.08
        _SaturationVariation("Saturation Variation", Range(0, 1)) = 0.08
        _BrightnessVariation("Brightness Variation", Range(0, 1)) = 0.12
        [Toggle(_RETRO_FOLIAGE_MOBILE)] _FoliageMobileLighting("Mobile Lighting", Float) = 0
    ...
    SubShader
    {
        ...
        CBUFFER_START(UnityPerMaterial)
            ...
            half4 _RimColor;
            half4 _BackfaceTint;
            half4 _FoliageTopColor;
            half4 _FoliageBottomColor;
            ...
            half _WindUVHeightMask;
            half _FoliageVariationStrength;
            half _HueVariation;
        ...
        float3 RetroApplyFoliageWind(float3 positionOS, float3 normalOS, float4 tangentOS, float2 uv, float4 vertexColor)
        {
            #if defined(_RETRO_FOLIAGE_WIND)
                half vertexMask = lerp(1.0h, vertexColor.a, saturate(_WindVertexColorMask));
                positionOS += BorzRetroWindOffsetOS(positionOS, normalOS, tangentOS, uv, vertexMask, _WindStrength, _WindDistance, _WindSpeed, _WindScale, _WindDirection, _WindUVHeightMask);
            #endif
            return positionOS;
        }
        ...
        half3 RetroApplyFoliageColor(half3 color, float2 uv, float3 positionWS)
        {
            #if defined(_RETRO_FOLIAGE_GRADIENT)
                half3 gradient = lerp(_FoliageBottomColor.rgb, _FoliageTopColor.rgb, saturate(uv.y));
                color *= gradient;
            #endif

            #if defined(_RETRO_FOLIAGE_VARIATION)
                half hash = BorzRetroHash12(floor(positionWS.xz * 0.35h));
                half brightness = lerp(1.0h - _BrightnessVariation, 1.0h + _BrightnessVariation, hash);
                half saturation = lerp(1.0h - _SaturationVariation, 1.0h + _SaturationVariation, frac(hash * 1.37h));
                half3 luminance = Luminance(color);
                color = lerp(luminance, color, saturation);
                color *= lerp(1.0h, brightness, _FoliageVariationStrength);
                color = lerp(color, color.gbr, _HueVariation * _FoliageVariationStrength * hash);
            #endif
        ...
        RetroVaryings RetroForwardVertex(RetroAttributes input)
        {
            ...
            float3 positionOS = RetroApplyFoliageWind(input.positionOS.xyz, input.normalOS, input.tangentOS, input.texcoord, input.color);
            positionOS = RetroApplyVertexWobble(positionOS, input.normalOS);
        ...
        )
        {
            ...
            color.rgb = RetroApplyFoliageColor(color.rgb, input.uv, input.positionWS);
            #if defined(_RETRO_BACKFACE_TINT)
        ...
        Pass
        {
            ...
            #pragma shader_feature_local _RETRO_FOLIAGE_WIND
            #pragma shader_feature_local_fragment _RETRO_FOLIAGE_GRADIENT
            #pragma shader_feature_local_fragment _RETRO_FOLIAGE_VARIATION
            #pragma shader_feature_local_fragment _RETRO_FOLIAGE_MOBILE
    ...
    CustomEditor "Borzblade.RetroRenderToolkit.Editor.PSXPS2FoliageShaderGUI"
}
