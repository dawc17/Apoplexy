Shader "Borzblade/Retro Render Toolkit/PSX PS2 Hybrid Lit"
{
    Properties
    {
        ...
        [Toggle(_RETRO_VERTEX_SNAP)] _VertexSnapEnabled("Vertex Snap Enabled", Float) = 1
        _VertexSnapStrength("Vertex Snap Strength", Range(0, 1)) = 0.35
        ...
        [Toggle(_RETRO_VERTEX_WOBBLE)] _VertexWobbleEnabled("Vertex Wobble Enabled", Float) = 1
        _VertexWobbleStrength("Vertex Wobble Strength", Range(0, 1)) = 0.12
    ...
    SubShader
    {
        ...
        half4 RetroDepthOnlyFragment(RetroDepthVaryings input) : SV_Target
        {
            ...
            #if defined(_ALPHATEST_ON)
                Alpha(RetroSampleBase(input.uv).a, _BaseColor, _Cutoff);
            #endif
            BorzRetroClipVertexDrawDistance(input.positionWS, _VertexDrawDistanceEnabled, _VertexDrawDistance, _VertexDrawDistanceFade, input.positionCS, _DitherScale);
    ...
    CustomEditor "Borzblade.RetroRenderToolkit.Editor.PSXPS2HybridShaderGUI"
}
